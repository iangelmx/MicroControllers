...
class API_tmx
{
	...
	protected function conectaBD(){
		$db_host= "localhost";
		$db_user= "user01pdsPrepro";
		$db_password= "85xuAAX2WNe7GUpELs7";
	...
	*/
	...
	function llamada($peticion, $url='http://localhost/pdstelmex/api/rest.php'){ //http://psdtelmex.com.mx
	  $ch = curl_init($url);
	...
	function getProyectoDepCrit($idDependencia){
		$db = $this->conectaBD();
		//SELECT parent_item_id FROM `app_entity_30` where id='1'
		$query="SELECT parent_item_id FROM `app_entity_30` WHERE id='$idDependencia';";
        $resultado=mysqli_query($db,$query);
        $nRows = mysqli_num_rows($resultado);
        if($nRows>0){
            $n = mysqli_fetch_array($resultado,MYSQLI_ASSOC);
